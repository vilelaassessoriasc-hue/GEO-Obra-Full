...
def tasks_nearby(
    ...
    FROM public.fn_job_nearby_tasks_v3(:job_id, CAST(:radius_km AS NUMERIC), :limit, :status, :created_from, :created_to);
""").bindparams(
    ...
    bindparam("created_to", type_=satypes.DateTime())
            ...
            sql_cursor = text("""
                WITH j AS (
                    SELECT
                        jobs.geom,
                        ( CAST(:radius_km AS double precision) / (111.32 * GREATEST(COS(RADIANS(ST_Y(jobs.geom))), 0.01)) )::double precision AS deg
                    FROM public.jobs
                ...
                WHERE
                    ...
                    AND (
                        ...
                        ( CAST(:cur_d AS numeric), CAST(:cur_id AS int) )
                    )
                ORDER BY
                    t.geom <-> j.geom
                LIMIT :limit;
        ...
        raise HTTPException(status_code=500, detail=f"{type(e).___name_______}: {e}")
